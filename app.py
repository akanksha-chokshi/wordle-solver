...
def letter_freq():
    ...
    ordered_letters = [(k, v) for k, v in sorted(letter_dict.items(), key=lambda item: item[1], reverse = True)]
    return ordered_letters

ordered_letters = letter_freq()

def score_words (words, ordered_letters):
    word_scores = {}
    letters_explored = []
    for word in words:
        word_scores[word]= 0
        letters_explored = []
        for letter in word:
            for order in ordered_letters:
                if order[0] == letter:
                    if letter not in letters_explored:
                        word_scores[word] += order [1]
                        letters_explored.append(letter)
    ordered_words = [k for k, v in sorted(word_scores.items(), key=lambda item: item[1], reverse = True)]
...
# figure out for repeat letters
    ...
    for i in range (5):
        if guesslist[i] == guesswordlist[i]:
            green.append(guesslist[i])
        elif guesslist[i] in guesswordlist:
            yellow.append(guesslist[i])
        else:
            grey.append(guesslist[i])
            
    return (green, yellow, grey)

def remove_solutions (solutions, guess, green, yellow, grey):
    newsolutions = []
    for word in solutions:
        check = True
        for i in range (0, 5):
...
if guessword and submit: 
    while (guesses > 0):
        ...
        if g == "Guessed":
            ...
            st.write ("Number of Guesses Left: " + str(guesses))
